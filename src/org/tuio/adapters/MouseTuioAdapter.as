package org.tuio.adapters
{
	...
	public class MouseTuioAdapter extends AbstractTuioAdapter{
		private var _clampFiducialRotation:Boolean;
		...
		private function dispatchObjectMove(event:MouseEvent):void{
			...
			if(!this.rKey){
				...
				if(this.clampFiducialRotation){
					rotationVal = rotationVal%(2*Math.PI);
				}
		...
		public function get clampFiducialRotation():Boolean
		{
			return _clampFiducialRotation;
		}

		/**
		 * clamps the fiducial's rotation value to 360 degrees.
		 * @param value
		 *
		 */
		public function set clampFiducialRotation(value:Boolean):void
		{
			_clampFiducialRotation = value;
		}
